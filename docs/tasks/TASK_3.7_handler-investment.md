...
- [ ] `internal/handler/asset_handler.go` — `AssetHandler` struct + all handlers.
- [ ] `internal/handler/asset_handler_test.go` — unit tests with mocked `InvestmentService`.
- [ ] Route registration for all asset endpoints in `internal/server/routes.go`.
- [ ] Swaggo annotations on every handler function.
- [ ] `RequireRole("admin")` middleware on asset write endpoints.
...
- [ ] `GET /v1/assets/{id}` returns the global asset with tenant overrides applied (calls `GetAssetWithTenantConfig`).
- [ ] `POST /v1/assets` returns `403` for non-admin callers.
- [ ] `PUT /v1/me/asset-configs/{asset_id}` creates or updates the config (upsert semantics).
- [ ] `DELETE /v1/me/asset-configs/{asset_id}` soft-deletes and returns `204`.
- [ ] Swaggo annotations exist on every handler function.
