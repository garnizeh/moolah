...
- [ ] `internal/service/investment_service.go` — `InvestmentService` implementing `domain.InvestmentService`.
- [ ] `internal/service/investment_service_test.go` — unit tests with mocked repositories.
- [ ] Position CRUD methods: `CreatePosition`, `GetPosition`, `ListPositions`, `UpdatePosition`, `DeletePosition`.
- [ ] Receivable lifecycle: `MarkIncomeReceived(ctx, tenantID, eventID string)`, `CancelIncome(ctx, tenantID, eventID string)`.
- [ ] Portfolio methods: `GetPortfolioSummary`, `TakeSnapshot`.
- [ ] Constructor receives: `positionRepo`, `positionSnapshotRepo`, `positionIncomeRepo`, `portfolioSnapshotRepo`, `assetRepo`, `tenantAssetConfigRepo`, `accountRepo`, `auditRepo`, `currencyConverter`.
...
- [ ] `InvestmentService` implements the `domain.InvestmentService` interface.
- [ ] `CreatePosition` rejects accounts whose `type != "investment"`.
- [ ] `MarkIncomeReceived` returns `domain.ErrConflict` if `status != "pending"`.
- [ ] `CancelIncome` returns `domain.ErrConflict` if `status != "pending"`.
- [ ] `GetPortfolioSummary` uses `CurrencyConverter` to normalise to tenant base currency before summing.
- [ ] `TakeSnapshot` is idempotent (unique constraint on `(tenant_id, snapshot_date)` — log and return existing on conflict).
- [ ] All service methods pass `tenant_id` to repositories; cross-tenant access is impossible.
- [ ] Unit tests cover each business rule with mocked repos.
