...
- [x] `internal/platform/db/queries/positions.sql` — CRUD + income-schedule queries.
- [x] `internal/platform/db/queries/position_snapshots.sql` — create + list queries (append-only).
- [x] `internal/platform/db/queries/position_income_events.sql` — create + status-update + list queries.
- [x] `internal/platform/db/queries/portfolio_snapshots.sql` — create + list queries (append-only).
...
- [x] All four query files created with the named queries listed above.
- [x] `ListPositionsDueIncome` does NOT include `tenant_id` filter (scheduler processes all tenants globally).
- [x] `UpdateIncomeEventStatus` only allows transitioning records owned by `tenant_id`.
- [x] All active-data queries include `AND deleted_at IS NULL` where the table supports soft-delete.
- [x] `sqlc generate` completes without errors; generated files committed.
- [x] `make task-check` passes.
- [x] `docs/ROADMAP.md` row 3.11 updated to ✅ `done`.
...
| 2026-03-14 | —      | Implemented SQL queries, generated Go code, and updated mocks. |
